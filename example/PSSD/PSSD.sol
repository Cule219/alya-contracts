...
contract ThePossessed is ERC721A, ERC2981, Ownable {
    ...
    function mint(
        ...
        if (
            !verifySig(msg.sender, maxMintable, msg.value / quantity, signature)
        ) revert IncorrectSignature();
        if (totalSupply() + quantity > collectionSize) revert SoldOut();
        if (_numberMinted(msg.sender) + quantity > maxMintable)
            revert MaxMinted();
    ...
    // OWNER FUNCTIONS ---------
    function setBaseURIs(string memory blessed, string memory possessed)
        public
        onlyOwner
    {
        ...
        returns (string memory)
    {
        ...
        string memory baseURI = _isPossessed[tokenID]
            ? possessedBaseURI
            : blessedBaseURI;
        return
