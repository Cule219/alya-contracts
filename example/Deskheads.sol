...
// File: erc721a@3.3.0/contracts/IERC721A.sol
...
// File: erc721a@3.3.0/contracts/ERC721A.sol
...
// File: erc721a@3.3.0/contracts/extensions/IERC721AQueryable.sol
...
// File: erc721a@3.3.0/contracts/extensions/ERC721AQueryable.sol
...
contract Deskheads is ERC721AQueryable, Ownable, IERC721Receiver {
    uint256 public immutable MAX_PER_WALLET_CL = 2; // Collabs
    uint256 public immutable MAX_PER_WALLET_DL = 2; // Desklist
    uint256 public immutable MAX_PER_WALLET_PB = 3; // Public
    uint256 public immutable MAX_SUPPLY = 5555;
    uint256 public immutable PRICE = 0.069 ether;
    ...
    string public preRevealURI =
        "ipfs://QmQ9xAqaRMootJMxdCdjDSvwJ8H4ZshNKmvtqWhHJCC7Ev";

    // STAKING

    // token to time
    mapping(uint256 => uint256) private cumulativeWorkTime;
    ...
    // token to worker
    mapping(uint256 => Worker) private workplaceVault;
    ...
    modifier isSupplyExceeded(uint256 quantity) {
    ...
    function mintDeskhead(uint256 quantity, bool stake) internal {
        minters[msg.sender] += quantity;

        if (stake) {
            uint256 startingTokenId = _currentIndex;
    ...
    function mintCL(
        ...
        external
        payable
        isMerkleProofValid(merkleProof, merkleRootCL)
        isSupplyExceeded(quantity)
    {
        require(saleState == SaleState.WHITELIST, "WL sale disabled");
        require(
            quantity + minters[msg.sender] <= MAX_PER_WALLET_CL,
            "Exceeds MAX_PER_WALLET_CL"
        );
    ...
    function mintDL(
        ...
        external
        payable
        isMerkleProofValid(merkleProof, merkleRootDL)
        isSupplyExceeded(quantity)
    {
        require(saleState == SaleState.WHITELIST, "WL sale disabled");
        require(
            quantity + minters[msg.sender] <= MAX_PER_WALLET_DL,
            "Exceeds MAX_PER_WALLET_DL"
        );
        require(msg.value >= (PRICE * quantity), "Insufficient funds");
    ...
    function mintPublic(uint256 quantity, bool stake)
        external
        payable
        isSupplyExceeded(quantity)
    {
        ...
        require(
            quantity + minters[msg.sender] <= MAX_PER_WALLET_PB,
            "Exceeds MAX_PER_WALLET_PB"
        );
        require(msg.value >= (PRICE * quantity), "Insufficient funds");
        ...
        returns (string memory)
    {
        ...
        uint256 finalTokenId = (((_tokenId - 1) + tokenOffset) %
            totalSupply()) + 1;
    ...
    function addToVault(uint256 tokenId, address holder) internal {
        workplaceVault[tokenId] = Worker({
            owner: holder,
            startWorkTime: uint64(block.timestamp)
        });
    }

    function removeFromVault(uint256 tokenId) internal {
        delete workplaceVault[tokenId];
    }

    function sendToWork(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(
                workplaceVault[tokenId].owner == address(0),
                "Token already at work"
            );
            require(
    ...
    function backFromWork(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(
                workplaceVault[tokenId].owner != address(0),
            ...
            require(
                workplaceVault[tokenId].owner == msg.sender,
                "Token not owned by msg.sender"
            );

            cumulativeWorkTime[tokenId] =
                cumulativeWorkTime[tokenId] +
                (block.timestamp - workplaceVault[tokenId].startWorkTime);
    ...
    function getWorkTime(uint256 tokenId)
        external
        view
        returns (
            bool working,
    ...
    function getWorkerOwner(uint256 tokenId)
        external
        view
    ...
    function listWorkers(address account)
        public
        view
        returns (uint256[] memory)
    {
        ...
        uint256 j;
        for (uint256 i = 1; j != totalTokens; i++) {
    ...
    function mintPrivate(address _to, uint256 quantity)
        external
        onlyOwner
        isSupplyExceeded(quantity)
    {
